...
def initMsgCustom(bot_info_dict):
        ...
        for dictStrCustom_this in OlivaDiceLogger.msgCustom.dictStrCustom:
            if dictStrCustom_this not in OlivaDiceCore.msgCustom.dictStrCustomDict[bot_info_dict_this]:
                OlivaDiceCore.msgCustom.dictStrCustomDict[bot_info_dict_this][dictStrCustom_this] = (
                    OlivaDiceLogger.msgCustom.dictStrCustom[dictStrCustom_this]
                )
        for dictHelpDoc_this in OlivaDiceLogger.msgCustom.dictHelpDocTemp:
            if dictHelpDoc_this not in OlivaDiceCore.helpDocData.dictHelpDoc[bot_info_dict_this]:
                OlivaDiceCore.helpDocData.dictHelpDoc[bot_info_dict_this][dictHelpDoc_this] = (
                    OlivaDiceLogger.msgCustom.dictHelpDocTemp[dictHelpDoc_this]
                )
    ...
    # 注入映射
    OlivaDiceCore.userConfig.dictConfigKeyToConsoleSwitchMapping.update(
        OlivaDiceLogger.msgCustom.dictConfigKeyToConsoleSwitchMapping
    )
